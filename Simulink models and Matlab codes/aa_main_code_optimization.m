...
configuration_s=2;               %(1)FWD1, (2)FWD2, (3)RWD1, (4)RWD2, (5)AWD2, or (6)AWD4
    if configuration_s==1
        configuration='FWD1';
    elseif configuration_s==2
        configuration='FWD2';
    elseif configuration_s==3
        configuration='RWD1';
    elseif configuration_s==4
        configuration='RWD2';
    elseif configuration_s==5
        configuration='AWD2';
    elseif configuration_s==6
        configuration='AWD4';
    end
    
for motorinverter_select_s = 1:1:3      %(1)A, (2)B, (3)C
    if motorinverter_select_s==1
        motorinverter_select='A';
    elseif motorinverter_select_s==2
        motorinverter_select='B';
    elseif motorinverter_select_s==3
        motorinverter_select='C';
    end
    
    for battery_capacity = 30000:10000:120000
        for drive_ratio = 1:1:10
            i=i+1;
w=0.090;                            %(m) frame rail width
h=0.180;                            %(m) frame rail height
...
motorf_X_dis = 0.322;                   % Motor         (front)       
inverterf_X_dis = 0.322;                % Inverter      (front)
gearboxf_X_dis = 0.4;                   % Gearbox       (front)
differentialf_X_dis = 0.4;              % Differential  (front)
frontaxle_X_dis = 0.622;                % Axle          (front)
batteryfront_X_dis = 0.7;               % Battery       (front)
infotainment_X_dis = 0.9;               % Infortainment 
frontpass_X_dis = 1.9;                  % Passenger     (front)
sunroof_X_dis = 2.0;                    % Sunroof       
body_X_dis = L108/2;                    % Body          
rearpass_X_dis = 2.8;                   % Passenger     (rear)
motorr_X_dis = 3.022;                   % Motor         (rear)
inverterr_X_dis = 3.022;                % Inverter      (rear)
gearboxr_X_dis = 3.1;                   % Gearbox       (rear)
differentialr_X_dis = 3.1;              % Differential  (rear)
rearaxle_X_dis = 3.322;                 % Axle          (rear)
batteryrear_X_dis = 3.0;                % Battery       (rear)
cargorear_X_dis = 3.6;                  % Cargo         (rear)
...
load_batteryf= -battery_mass*g/2;
...
load_batteryr= -battery_mass*g/2;
...
elseif lateral_acceleration_max<(g*0.8)
    turning_result=0;    %Increase the speed and steering angle
end
...
%% Loops end (comment if not needed)
% each row represents one iteration
% configuration; motor type; battery capacity; drive ratio; 
% profit; cost ; 0-100 km/h ; top speed; battery range; 
% asecending 12% slope ; max stress ; max deflection; 
% kus value; lateral accelration value; turinging diameter value; VD pass/fail; spring
% constant; spring damping coeffiecint; CG location
             
            solutions(i,1)=configuration_s;
            solutions(i,2)=motorinverter_select_s;
            solutions(i,3)=battery_capacity;
            solutions(i,4)=drive_ratio;  
            solutions(i,5)=profit;
            solutions(i,6)=cost;
            solutions(i,7)=acceleration_time;
            solutions(i,8)=max_speed;
            solutions(i,9)=range_battery;
            solutions(i,10)=slope_result;
            solutions(i,11)=stress;
            solutions(i,12)=deflection_max;
            solutions(i,13)=Kus;
            solutions(i,14)=lateral_acceleration_max;
            solutions(i,15)=diameter;
            solutions(i,16)=turning_result;
            solutions(i,17)=spring_rate_front;
            solutions(i,18)=spring_rate_rear;
            solutions(i,19)=spring_damping_coeffiecient_front;
            solutions(i,20)=spring_damping_coeffiecient_rear;
        
          end
      end
 end

solutions_header={'configuration', 'motorinverter_select', 'battery_capacity', 'drive_ratio' ...
    'profit' 'cost' 'acceleration_time 0-100 km/h' 'max_speed' 'range_battery' ...
    'slope_result' 'stress' 'deflection_max' 'Kus' 'lateral_acceleration_max' ...
    'diameter' 'turning_result' 'spring_rate_front' 'spring_rate_rear' ...
    'spring_damping_coeffiecient_front' 'spring_damping_coeffiecient_rear'};